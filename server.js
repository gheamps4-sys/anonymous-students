...
// ✅ SECRET PAGE: See ALL student IDs + quick preview
app.get('/all-ids', (req, res) => {
  const all = loadDB();
  let list = `<html style="font-family:Arial; padding:2rem; max-width:700px; margin:0 auto;">
  <h2>📋 All Student Message IDs</h2><hr><ul style="line-height:1.8;">`;

  if(all.length === 0) {
    list += <li>No messages yet.</li>;
  } else {
    all.forEach((m, i) => {
      list += `<li><strong>ID:</strong> <code>${m.id}</code> — <strong>Posted:</strong> ${new Date(m.time).toLocaleString()}<br>
      <em>Preview:</em> ${m.text.substring(0,60)}… | <a href="/admin">Reply</a></li><br>`;
    });
  }

  list += </ul><hr><p><a href="/admin">← Go to Reply Panel</a> | <a href="/">← Home</a></p></html>;
  res.send(list);
});